...
**Input Parameters**
- **Intrinsic dynamics** (`sigma<sub>int</sub>`): sets the intrinsic velocity dispersion of the Lya photons from the emitting source [as sigma_int/(km * s<sup>-1</sup>)]. The photons are sampled from a Gaussian with this width.
...
- **Expansion velocity** (`v<sub>exp</sub>`): sets the inflow/outflow velocity of the shell [as v_exp/(km * s<sup>-1</sup>)]. This is uniform throughout the shell.
